...
import fs from "fs" // Required to delete local files
...
const generateAccessAndRefereshToken = async (userId) => {
   try {
    ...
    // 2. Generate tokens using the methods defined in your model
    ...
    // We use validateBeforeSave: false because we don't want 
    // to re-run validation (like password checks) just for a token update
    await user.save({ validateBeforeSave: false });

    // 4. Return the tokens to the calling function
...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if (coverImageLocalPath) {
        try {
            ...
            // Optional: If cover image fails but avatar succeeded, 
            // you might want to delete avatar from Cloudinary before throwing error
    ...
    try {
        ...
        if (!createdUser) {
            throw new ApiError(500, "Internal Server Error: User creation failed in database")
        }
        ...
        throw new ApiError(500, error?.message || "User registration failed. Please try again.")
    }
})
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    //check whegther we have loggedin user or not
    const options = {
        httpOnly: true,
